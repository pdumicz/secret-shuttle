...
- **Baseline timing (Finding 2) — CLOSED via dual-sample union.** The baseline is sampled at BOTH the pre-approval point (agent still observing) AND post-sever immediately before the reveal click; `RESOLVE_SCAN_FN`'s reject set is the UNION of both samples' `readableFps`. A value script-observable at either time fails closed, closing both the realistic approval-window staleness hole and the adversarial agent-JS-survives-sever erase window. The only remaining (accepted, detection-only, agent-already-has-the-secret) residual here is a value that exists *strictly between* the two samples and is present at neither — doubly-adversarial and transient; out of scope for the same reason as the changed-readable+path-shift residual above.
