...
- EventSource to /ui/hub/stream with explicit close-on-error,
  ...
  banner with in-page Reconnect button (takeOver() flips terminal,
  resets the counter, calls connect() — does NOT instruct a reload,
  which would hit /ui/hub with no token after history.replaceState).
