...
test("listCommand subprocess: stderr is a single JSON document with error_code AND warning when daemon is down", async () => {
  ...
  const tmp = await mkdtemp(path.join(os.tmpdir(), "shuttle-list-shim-test-"));
  try {
    const res = spawnSync(process.execPath, ["dist/cli/index.js", "list"], {
      env: {
        ...process.env,
    ...
    assert.equal(warning.deprecated, "list");
  } finally {
