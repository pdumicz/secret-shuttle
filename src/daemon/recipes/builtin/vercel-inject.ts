...
export const vercelInject: InjectRecipe = {
  ...
  // URL with runtime placeholders for team and project.
  //
  // The `{team}` / `{project}` placeholders are substituted at runtime by
  // `interpolateUrl` from the user's yml `url_params`. Missing values fail-closed
  // with `recipe_url_params_missing` BEFORE any browser side-effect — no risk of
  // routing to the wrong project.
  //
  // Selectors below are still best-effort pending real-page dogfood verification
  // (see `verified_against_real_page` below); URL addressability is decoupled from
